        ...
        if (lc_valid_in_reg) begin
          ...
          for (int i = 0; i < MSHR_COUNT; i++) begin
            if (!mshr_outputs[i].valid) begin
              // there is a free mshr entry, we can take the request
              lsu_ready_out_comb = 1;
              break;
            end else if (mshr_outputs[i].no_offset_addr == no_offset_addr) begin
              // matches an existing MSHR, check if its queue has space
              if (!mshr_full[i]) begin
                lsu_ready_out_comb = 1;
                break;
              end
            end
          ...
          if (!mshr_full[pos]) begin
            if (lsu_we_in_reg) begin
              ...
              for (int i = 0; i < 16; i++) begin
                if (mshr_queue_full[pos][i].valid && mshr_queue_full[pos][i].paddr == cur_addr && mshr_queue_full[pos][i].we) begin
                  // Found a matching write in the MSHR queue, forward the data!
                  lsu_value_out_comb = mshr_queue_full[pos][i].data;
                  needToAdd = 0;
                  next_state = SEND_RESP_HC;
                  // $display("MSHR Forwarding: forwarding value 0x%h to tag %d", lsu_value_out_comb, lsu_tag_in_reg);
                  break;
                end
              end
