      ...
      if (queue[s].valid && (queue[s].op_type == OP_STORE) && !queue[s].dispatched) begin
        ...
        if (is_older(s, store_idx)) begin
          ...
          if (queue[s].addr == store_addr) begin
            ...
            // Forwarding for WAW: The younger store (store_idx) is now ready to complete
            // because an older store to the same address exists and will be processed first.
            // We MUST NOT overwrite the younger store's value with zero.
...
module memory_interface #(
  ...
  always_ff @(posedge clk_in) begin
    if (!rst_N_in) begin
      ...
      dispatch_ready_out <= 1'b0;
    end else begin
      // Default latch values
      lat_is_store <= lat_is_store;
      lat_addr     <= lat_addr;
      lat_value    <= lat_value;
      lat_tag      <= lat_tag;
      ...
      // l1d_ready_out logic (fixed blocking to non-blocking)
